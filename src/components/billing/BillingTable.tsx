...
import { formatBillingDateRange } from "@/lib/billingSummary";
import {
...
export function BillingTable({
  ...
  const periodLabel = formatBillingDateRange(fromDate, toDate);

  if (rows.length === 0) {
    return (
      <p className="py-12 text-center text-sm text-gray-500">
        No tenants with bills in the selected period.
      </p>
    );
  }
      ...
      <table className="w-full min-w-[760px] text-left text-sm">
        <thead>
          <tr className="border-b border-gray-200 bg-white">
            ...
            <th className="px-4 py-3.5 text-sm font-semibold text-navy">Paid</th>
            <th className="px-4 py-3.5 text-sm font-semibold text-navy">
              Balance
            </th>
            <th className="px-5 py-3.5 text-sm font-semibold text-navy">Status</th>
          </tr>
        </thead>
        <tbody>
          {rows.map((row) => {
            const isSelected = selectedRow?.room === row.room;
            ...
            return (
              ...
              >
                ...
                <td className="px-4 py-4 text-gray-600">{periodLabel}</td>
